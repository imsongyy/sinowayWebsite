...
public class GenerateLinkUtils {
	...
	private final static Logger logger = Logger.getLogger(GenerateLinkUtils.class);

	private static final String CHECK_CODE = "checkCode";

	public static String generateActivateLink(CustomerEntity user) {
		
		return ConfigUtils.getValue("serverAddress")+"/activate?id="+user.getId()+"&"+CHECK_CODE+"="+generateCheckcode(user);
	}
	 ...
	 * @return
	 */
	private static String generateCheckcode(CustomerEntity user) {
		String userName = user.getOrgFullname();
		Long randomCode = user.getId();
	...
	private static String md5(String string) {
		...
		try {
			...
			e.printStackTrace();
			logger.error("md5这里出错了,"+e.getMessage());
		}
		return null;
	}
	...
	public static boolean verifyCheckcode(CustomerEntity user, String checkCode) {
